...
export default class NewListClass extends React.Component {
  ...
  getData = () => {
    let result = [];
    axios({
      url: baseURL+"/panel-fetch-other-delivery",
      method: "GET",
      headers: {
      ...
      .then((res) => {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            tempRows.push([
              ...
              (response[i]["delivery_shipping_date"] == null ? "" : Moment(response[i]["delivery_shipping_date"]).format('DD-MM-YYYY')),
              (response[i]["delivery_shipping_date"] == null ? "" : Moment(response[i]["delivery_date"]).format('DD-MM-YYYY')),
