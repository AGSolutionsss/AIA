...
const NotAttendReport = (props) => {
  ...
  useEffect(() => {
    ...
    axios({
      url: baseURL+"/fetch-notattend-report",
      method: "POST",
      data,
      headers: {
    ...
    setIsButtonDisabled(true)
      axios({
        ...
        headers: {
          ...
          link.href = url;
          link.setAttribute('download', 'not_attend_list.csv'); 
  ...
  return (
    <div>
      ...
      <div className="invoice-wrapper">
        <PageTitleBar title="Not Attend List" match={props.match} />
        <div className="row">
                ...
                <div className="d-flex justify-content-between" style={{fontSize: '16px' }}>
                  ...
                  <div className="address text-center">
                    ...
                    <h2 style={{paddingTop: '5px'}}>
                      <strong>
                        <b style={{fontSize: '20px'}}>Not Attend List</b>
                      </strong>
                    </h2>
                  </div>
                ...
                <div className="table-responsive">
                  <TableContainer component={Paper}>
                    <Table aria-label="simple table" style={{border: '2px solid black', marginTop: '20px'}}>
                      <TableHead>          
                        <TableRow>
                          <TableCell style={table_head}>Class Date</TableCell>
                          ...
                          <TableCell style={table_head}>Class Time</TableCell>
                         </TableRow>
                      </TableHead>
                      <TableBody>
                        {teamsummary.map((dataSumm, key)=>(
                          <TableRow key={key}>
                            <TableCell style={table_row_count}>{Moment(dataSumm.class_date).format('DD-MM-YYYY')}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.user_uid}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.name}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.class_subject}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.class_time}</TableCell>
                          </TableRow>
                        ))}
                       </TableBody>
                    </Table>
...
export default NotAttendReport;